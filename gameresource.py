...
class GameResource():
    ...
    def load_newgame_img(self):
        if not self.newgame_img:
            self.newgame_img = pygame.image.load(
                #self.img_path + "press-s-newgame.png").convert_alpha()
                self.img_path + "press-s-q-newgame.png").convert_alpha()
        return self.newgame_img

    def load_pausing_img(self):
        if not self.pausing_img:
            self.pausing_img = pygame.image.load(
                self.img_path + "game-p-pausing.png").convert_alpha()
        return self.pausing_img

    def load_continue_img(self):
        if not self.continue_img:
            self.continue_img = pygame.image.load(
                self.img_path + "game-p-q-pausing-exit.png").convert_alpha()
        return self.continue_img

    def load_gameover_img(self):
        if not self.gameover_img:
            self.gameover_img = pygame.image.load(
                self.img_path + "game-over.png").convert_alpha()
        return self.gameover_img

    def play_bg_music(self):
        pygame.mixer.init()
        bg_music_file = self.music_path + 'bg_music_0.mp3'
    ...
    def load_bg_img(self):
        if not self.bg_img:
            self.bg_img = pygame.image.load(self.img_path + "bg-pig.jpg")
